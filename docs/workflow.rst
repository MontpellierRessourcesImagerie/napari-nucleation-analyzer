...
1. Detection and tracking of centrioles
=======================================

a. Detect centrioles
--------------------

- To make the detection of the centrioles and easier task, several filters are applied time 
  point per time point to the whole image.
  
  1. A median filter with a square kernel of 3×3 pixels is applied to reduce noise.
  2. The background is estimated and subtracted using morphological opening bigger than a centriole.
  3. A Laplacian of Gaussian filter is applied to enhance the centrioles.
...
This step produces a collection of 2D points for each frame that are candidates for being centrioles. 
There can still be false positives or centrioles you are not interested in.

b. Track the centrioles
-----------------------

The tracking of centrioles relies on [TrackPy]_, a Python library for particle tracking.
The `nearest velocity algorithm`_ is used to link the detected centrioles over time.
The "memory" parameter determines for how many frames a centriole can disappear without changing 
of identity. The "searching range" parameter determines how far a centriole can move between two consecutive
time points before it is considered as a new centriole. 

The higher these two parameters, the better centrioles will be tracked but the more likely it is to make mistakes.

At the end of this step, we have a collection of trajectories (== a 2D coordinate for each time point) 
that are candidates for being centrioles.


c. From detected centrioles to centrosomes
------------------------------------------

The only thing allowing to know which centrioles are bound together to form a centrosome 
and which one are worth analyzing are the hints that you provided.

For each hint:

- We take the centrioles candidates at the designated starting point as well as the hints for 
  this same time point.
- For each hint point, we look at the closest candidate centriole. If it is closer than the 
  "max binding distance" parameter, we link this centriole to the hint and we consider it as a 
  centriole of interest. By finding the candidate at this time point, we find it for the whole 
...
- To have a dense and uniform sampling, missing points on trajectories are interpolated using a 
  linear interpolation. This is done for each centriole of interest.
- The trajectories are filtered to keep only the time points from the requested time range.

At this moment, we have a trajectory for each centriole of interest. Each centriole has a unique ID 
and each centrosome has a unique ID. Thanks to the hints, each centriole ID is associated to a centrosome ID. 
The two centrioles of a same centrosome are linked together by a line in the Napari viewer.
...
- **centriole_id**: the centriole ID
- **X**: the X coordinate of the centriole in pixels
- **Y**: the Y coordinate of the centriole in pixels
- **track_id**: a duplication of the centriole ID, used for compatibility with Napari track layers.

2. Build arcs from centrioles
=============================

a. Process vectors
------------------

At this point, we have for each centrosome, the pairs of points corresponding to the two centrioles. 
To know towards which direction the arcs should be built, we need to compute a direction vector
for each of them. If we say that C1 is the position of a centriole and C2 the position of the other, 
...
between the X-axis' vector and the direction vector of the centrioles. We name this angle 𝛽.
...
it around the origin to be oriented along the direction vector of the centrioles.

.. math::

   R = \begin{bmatrix} \cos\beta & \sin\beta \\ -\sin\beta & \cos\beta \end{bmatrix}

The last step consists in translating the arc to be centered on the centrioles. This is a simple
addition of the coordinates of the centrioles to the coordinates of the points on the arc.

We just produced for each centriole of interest and for each point of time, a collection of points 
...
We have arcs center on centrioles and oriented along the direction vector of the centrioles. 
...
The same prominence-based detection algorithm that was used to detect centrioles is used to detect spots on the kymographs.
The only difference is that the prominence is computed differently. 
The base prominence is equal to the standard deviation of the pixel values on the kymograph. 
The final prominence is this base prominence multiplied by a user-defined factor.
