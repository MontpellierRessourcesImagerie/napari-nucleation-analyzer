...
class ExportArchiveOperator:
    ...
    def _gather_pairs_tracks(self, folder: Path):
        tracks = {}
        o = self.widget.tracks_manager_widget
        for pair_id, row in o._rows.items():
            item = {
            ...
            tracks[pair_id] = item
        with open(folder / "pairs_tracks.json", "w") as f:
            json.dump(tracks, f)

    def _gather_pairs_lines(self, folder: Path):
        o = self.widget.tracks_manager_widget
        lines_folder = folder / "pairs_lines"
        lines_folder.mkdir(exist_ok=True)

        for pair_id, row in o._rows.items():
            layer_name = o.as_pair_layer_name(pair_id)
            ...
            line = layer.data
            np.save(lines_folder / f"{pair_id}.npy", line)

    def _gather_tracked_pairs(self, folder: Path):
        image_layer_name = self.widget.image_combo.currentText()
        tracked_pairs_layer_name = self.widget.centrosomes_tracks_prefix + image_layer_name
        tracked_pairs_layer = (
            self.viewer.layers[tracked_pairs_layer_name] 
            if tracked_pairs_layer_name in self.viewer.layers 
            else None
        )
        if tracked_pairs_layer is None:
            raise ValueError(f"Tracked pairs layer '{tracked_pairs_layer_name}' not found in the viewer.")
        df = tracked_pairs_layer.features
        df.to_csv(folder / "tracked_pairs.csv", index=False)
    ...
    def _gather_arcs(self, folder: Path):
        ...
        for centrosome_id, arc in arcs.items():
            np.save(arcs_folder / f"{centrosome_id}.npy", arc)
    ...
    def _gather_kymographs(self, folder: Path):
        ...
        for centrosome_id, kymo in kymographs.items():
            tiff.imwrite(kymos_folder / f"{centrosome_id}.tif", kymo)
    ...
    def _gather_spots(self, folder: Path):
        ...
        for centrosome_id, spot in spots.items():
            np.save(spots_folder / f"{centrosome_id}.npy", spot)
    ...
    def run(self):
        ...
        self._gather_pairs_tracks(buffer_folder)
        self._gather_tracked_pairs(buffer_folder)
        self._gather_pairs_lines(buffer_folder)
