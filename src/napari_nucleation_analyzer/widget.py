...
from .pairs_manager_widget import PairsManagerWidget
from .export_archive_operator import ExportArchiveOperator

class CentrosomePairsWidget(QWidget):

    centrosomes_tracks_prefix = "_Tracked-centrosomes-"
    ...
    def set_tracks_panel(self, parent_layout):
        self.tracks_group = QGroupBox("Pair tracks")
        layout = QVBoxLayout()

        self.tracks_manager_widget = PairsManagerWidget(self.viewer, self)
        layout.addWidget(self.tracks_manager_widget)

        self.tracks_group.setLayout(layout)
        parent_layout.addWidget(self.tracks_group)

    def find_pairs_panel(self, parent_layout):
        self.find_pairs_group = QGroupBox("Find pairs")
        layout = QVBoxLayout()
        ...
        self.prominence_input.setValue(FindPairsOperator.default_prominence())
        ...
        self.searching_range_input.setValue(FindPairsOperator.default_searching_range())
        ...
        self.memory_input.setValue(FindPairsOperator.default_memory())
        ...
        self.max_binding_distance_input.setValue(FindPairsOperator.default_max_binding_distance())
        ...
        # Find pairs
        self.find_pairs_button = QPushButton("Find pairs")
        self.find_pairs_button.setFont(self.custom_font)
        self.find_pairs_button.clicked.connect(self.launch_find_pairs)
        layout.addWidget(self.find_pairs_button)

        self.find_pairs_group.setLayout(layout)
        parent_layout.addWidget(self.find_pairs_group)
    ...
    def launch_find_pairs(self):
        ...
        def runner():
            try:
                ...
                show_warning(f"Error during pair finding: {str(e)}")
                self.set_enabled(True)
                self.current_operator = None
        ...
        worker.finished.connect(self.finished_find_pairs)
        worker.start()

    def finished_find_pairs(self, *args):
        if self.current_operator is None:
            return

        self.set_enabled(True)

        if not isinstance(self.current_operator, FindPairsOperator):
            raise ValueError("Current operator is not a FindPairsOperator.")
        
        image_layer_name = self.image_combo.currentText()
        pairs_tracks_layer_name = self.centrosomes_tracks_prefix + image_layer_name
        image_layer = self.viewer.layers[image_layer_name]

        result = self.current_operator.get_pairs()
        required_cols = ["centrosome_id", "T", "Y", "X", "pair_id"]
        if not all(col in result.columns for col in required_cols):
            raise ValueError(f"Resulting pairs dataframe must contain columns: {required_cols}")
        ...
        if pairs_tracks_layer_name in self.viewer.layers:
            layer = self.viewer.layers[pairs_tracks_layer_name]
            layer.data = result[['centrosome_id', 'T', 'Y', 'X']]
        ...
        pair_ids, lines, colors = FindPairsOperator.as_lines(result, track_colors)

        for c_id, line, color in zip(pair_ids, lines, colors):
            layer_name = self.tracks_manager_widget.as_pair_layer_name(c_id)
    ...
    def launch_build_arcs(self):
        tracked_pairs_layer_name = self.centrosomes_tracks_prefix + self.image_combo.currentText()
        if tracked_pairs_layer_name not in self.viewer.layers:
            show_warning("No tracked pairs layer found. Please run 'Find pairs' first.")
            return
        tracked_pairs_layer = self.viewer.layers[tracked_pairs_layer_name]
        tracked_pairs = tracked_pairs_layer.features
    ...
    def _distances_as_features(self, pair_id, df, scale):
        df = df[df['pair_id'] == pair_id]
        if df.empty:
            raise ValueError(f"No centrosomes found for pair_id {pair_id}.")
        df = df.sort_values(by=['centrosome_id', 'T'])
        df = df.iloc[len(df) // 2:]
        df = df['distance'] * scale['X'][0]
        df = df.reset_index(drop=True)
        return df

    def _add_pair_distance(self, pair_id, df, scale):
        distances = self._distances_as_features(pair_id, df, scale)
        pair_layer_name = self.tracks_manager_widget.as_pair_layer_name(pair_id)
        pair_layer = self.viewer.layers[pair_layer_name]
        pair_layer.features['distance'] = np.round(distances, 2)
        unit = scale['X'][2]
        text = {
    ...
    def finished_build_arcs(self, *args):
        ...
        matchs = self.current_operator.get_matchs(flipped=True)
        df = self.current_operator.get_pairs()

        image_layer_name = self.image_combo.currentText()
        image_layer = self.viewer.layers[image_layer_name]
        image_layer.metadata['centrosomes-matchs'] = matchs
        dims = self._get_image_dims()

        centrosomes_layer_name = self.centrosomes_tracks_prefix + image_layer_name
        centrosomes_layer = self.viewer.layers[centrosomes_layer_name]
        centrosomes_layer.features = df

        for centrosome_id, arc in arcs.items():
            pair_id = int(matchs[centrosome_id])
            color = self.tracks_manager_widget._rows[pair_id].color
            layer_name = f"{self.arcs_shapes_prefix}{centrosome_id}"
            if layer_name in self.viewer.layers:
                layer = self.viewer.layers[layer_name]
                self.viewer.layers.remove(layer)
            self._add_pair_distance(pair_id, df, dims)
            self.viewer.add_shapes(
                arc,
                shape_type='path',
                edge_color=color,
                ...
                opacity=0.75,
                name=f"{self.arcs_shapes_prefix}{centrosome_id}",
                scale=image_layer.scale,
                units=image_layer.units
            )
    ...
    def _gather_arcs(self):
        ...
        for layer in self.viewer.layers:
            if layer.name.startswith(self.arcs_shapes_prefix):
                centrosome_id = int(layer.name.replace(self.arcs_shapes_prefix, ""))
                arcs[centrosome_id] = np.array(layer.data)
        return arcs
    ...
    def launch_build_kymographs(self):
        ...
        pairs_layer_name = self.centrosomes_tracks_prefix + image_layer_name
        if pairs_layer_name not in self.viewer.layers:
            show_warning("No tracked pairs layer found. Please run 'Find pairs' first.")
            return
        ...
        pairs = self.viewer.layers[pairs_layer_name].features
        self.set_enabled(False)
    ...
    def finished_build_kymographs(self, *args):
        ...
        image_layer_name = self.image_combo.currentText()
        image_layer = self.viewer.layers[image_layer_name]
        matchs = image_layer.metadata.get('centrosomes-matchs', None)

        if matchs is None:
            raise ValueError("Centrosomes matchs not found in image layer metadata. Please run 'Build arcs' first.")

        centrosome_ids = [(k, v) for k, v in matchs.items()]
        ...
        colors = [self.tracks_manager_widget._rows[int(matchs[centrosome_id])].color for centrosome_id in centrosome_ids]
        ...
        for i, centrosome_id in enumerate(centrosome_ids):
            kymograph = kymographs[centrosome_id]
            name = f"{self.kymo_prefix}{centrosome_id}"
    ...
    def _gather_kymographs(self):
        ...
        for layer in self.viewer.layers:
            if layer.name.startswith(self.kymo_prefix):
                centrosome_id = int(layer.name.replace(self.kymo_prefix, ""))
                kymographs[centrosome_id] = layer.data
        return kymographs

    def launch_locate_spots(self):
        all_kymos_layer_names = [layer.name for layer in self.viewer.layers if layer.name.startswith(CentrosomePairsWidget.kymo_prefix)]
        if not all_kymos_layer_names:
            show_warning("No kymograph layers found. Please run 'Build kymographs' first.")
            return
        
        kymographs = {int(layer.name.replace(CentrosomePairsWidget.kymo_prefix, '')): self.viewer.layers[layer.name].data for layer in self.viewer.layers if layer.name.startswith(CentrosomePairsWidget.kymo_prefix)}
        self.set_enabled(False)
    ...
    def finished_locate_spots(self, *args):
        ...
        for centrosome_id, coords in coordinates.items():
            ...
            kymo_layer_name = f"{CentrosomePairsWidget.kymo_prefix}{centrosome_id}"
            if kymo_layer_name not in self.viewer.layers:
                show_warning(f"Kymograph layer for track {centrosome_id} not found.")
                continue
            
            kymo_layer = self.viewer.layers[kymo_layer_name]
            spots_layer_name = f"{CentrosomePairsWidget.spots_layer_prefix}{centrosome_id}"
            if spots_layer_name in self.viewer.layers:
                layer = self.viewer.layers[spots_layer_name]
                self.viewer.layers.remove(layer)
            self.viewer.add_points(
                coords,
                name=f"{CentrosomePairsWidget.spots_layer_prefix}{centrosome_id}",
    ...
    def _gather_spots(self):
        ...
        for layer in self.viewer.layers:
            if layer.name.startswith(self.spots_layer_prefix):
                centrosome_id = int(layer.name.replace(self.spots_layer_prefix, ""))
                spots[centrosome_id] = layer.data
        return spots
    ...
    def launch_export_summary(self):
        ...
        all_spots_layer_names = [layer.name for layer in self.viewer.layers if layer.name.startswith(CentrosomePairsWidget.spots_layer_prefix)]
        ...
        kymographs = {int(layer.name.replace(CentrosomePairsWidget.kymo_prefix, '')): self.viewer.layers[layer.name].data for layer in self.viewer.layers if layer.name.startswith(CentrosomePairsWidget.kymo_prefix)}
        spots = {int(layer.name.replace(CentrosomePairsWidget.spots_layer_prefix, '')): self.viewer.layers[layer.name].data for layer in self.viewer.layers if layer.name.startswith(CentrosomePairsWidget.spots_layer_prefix)}
        pairs_df = self.viewer.layers[tracked_pairs_layer_name].features

        self.set_enabled(False)

        op = SummaryOperator()
        op.set_pairs(pairs_df)
        op.set_kymographs(kymographs)
        op.set_spots(spots)
...
def run():
    ...
    widget = CentrosomePairsWidget(viewer=viewer)
    ...
    # pairs_path = dump / "pair_points_track.csv"
    # pairs = pd.read_csv(pairs_path)

    # viewer.add_tracks(
    #     pairs[['track_id', 'T', 'Y', 'X']],
    #     name=CentrosomePairsWidget.pairs_tracks_prefix + im_layer_name,
    #     scale=(1, calib, calib),
    #     features=pairs,
    #     graph=None,
    #     tail_length=4,
    ...
    # viewer.add_points(
    #     pairs[['T', 'Y', 'X']].values,
    #     name=CentrosomePairsWidget.pairs_points_prefix + im_layer_name,
    #     scale=(1, calib, calib),
    #     properties=pairs.drop(columns=['T', 'Y', 'X']).to_dict(orient='list'),
    #     border_color='track_id',
    #     face_color='transparent',
    #     size=15,
    #     border_color_cycle=widget.colors[:len(pairs['track_id'].unique())]
    # )
    ...
    # viewer.add_shapes(
    #     as_napari_shapes,
    #     name=CentrosomePairsWidget.arcs_shapes_prefix + im_layer_name,
    #     shape_type='path',
    #     edge_color='track_id',
    #     features=features,
    #     edge_width=2,
